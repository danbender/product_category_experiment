...
(function() {
  ...
  function init() {
    ...
    // back to intro after transition:
    var onEndTransFn = function() {
        this.removeEventListener( transEndEventName, onEndTransFn );
        classie.add( splitlayout, 'reset-layout' );
        document.body.scrollTop = document.documentElement.scrollTop = 0;
      },
      backToIntro = function( event ) {
        event.preventDefault();
        event.stopPropagation();
        var dir = classie.has( event.target, 'back-right' ) ? 'left' : 'right',
...
// Clone product on click to product view page
